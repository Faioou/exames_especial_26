...
### Critério 2 — SE HOUVER (PODE NÃO HAVER) um não terminal do lado direito, só pode haver um

Se houver um não terminal no lado direito, então:

```text
1. Só pode haver UM não terminal.
2. Esse não terminal tem de estar numa extremidade:
   - no fim  → gramática linear à direita
   - no início → gramática linear à esquerda
```

Exemplos permitidos:

```text
A → aB      ✅ não terminal no fim
A → Ba      ✅ não terminal no início
A → a       ✅ só terminal
A → ε       ✅ cadeia vazia
```

Não permitidos como Tipo 3:

```text
A → aBC     ❌ dois não terminais
A → aBb     ❌ B está no meio
A → BC      ❌ dois não terminais
```

Portanto, o teu critério pode ficar assim:

> **Se houver um não terminal no lado direito, só pode haver um e tem de estar numa das extremidades da produção.**


#### Critério 2.1 —  Gramática linear à direita, o não terminal tem de aparecer no fim
...
#### Critério 2.2 —  Gramática linear à esquerda, o não terminal tem de aparecer no início
...
### Critério 3 — pode existir uma produção só com um terminal
...
### Critério 4 — `ε` é permitido, mas NÃO é obrigatório

...
# O que é LL(1) = Analisador Sintático/Parser Preditivo 
...
Temos, duas sequências de derivação:
...
## Porque é que isto prova ambiguidade?

Porque as **árvores de derivação são diferentes**.

Na primeira derivação,
...
# Ter duas sequências diferentes não signficia que temos duas àrvores de sequência diferentes

### Exemplo que NÃO é ambíguo
...
São duas sequências diferentes, porque numa substituíste primeiro `A` e na outra primeiro `B`.

Mas a árvore é a mesma:
...
```text
duas sequências de derivação ≠ necessariamente ambiguidade
```
...
# 5. Agora conseguimos perceber a definição/caracterização formal de uma gramática `G = (V, Σ, P, S)`
